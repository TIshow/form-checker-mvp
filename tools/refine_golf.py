#!/usr/bin/env python3
"""Refine saved commercial-candidate evidence locally; never overwrite raw data."""
import argparse
import hashlib
import json
...
def main():
    ...
    if (out/'report.json').exists():
        p.error(f'{out} already contains a run; choose a new --out')
    ...
    clips=[]
    for label,J,color in [('公式経路・補正前',W,0xe8a45e),('画像に合わせた補正・検証中',world,0x64c8b2)]:
